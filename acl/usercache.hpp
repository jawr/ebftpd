...
class UserCache
{
  ...
  typedef boost::unordered_map<uid_t, acl::User*> ByUIDMap;
...
public:
  ...
  static util::Error SetPrimaryGID(const std::string& name, gid_t primaryGID);
  static util::Error AddSecondaryGID(const std::string& name, gid_t secondaryGID);
  static util::Error DelSecondaryGID(const std::string& name, gid_t secondaryGID);
  ...
  static const acl::User User(uid_t uid);
};
