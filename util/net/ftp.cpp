...
int main()
{
  using namespace util::net::ftp;

  std::cout << AF_INET << " " << AF_INET6 << std::endl;
  std::string s = "127,0,0,1,255,255";
  Endpoint ep;
  assert(EndpointFromPORT(s, ep));
  std::cout << "from port v4: " << ep << std::endl;

  assert(EndpointToPORT(ep, s));
  std::cout << "to port v4: " << s << std::endl;
  
  EndpointToLPRT(ep, s);
  std::cout << "to lprt v4: " << s << std::endl;
  
  assert(EndpointFromLPRT(s, ep));
  std::cout << "from lprt v4: " << ep << std::endl;

  ep = Endpoint("::1", 1234);
  EndpointToLPRT(ep, s);
  std::cout << "to lprt v6: " << s << std::endl;
  
  util::Error e = EndpointFromLPRT(s, ep);
  if (!e) std::cout << e.Message() << std::endl;
  else std::cout << "from lprt v6: " << s << std::endl;
  
  EndpointToEPRT(ep, s, true);
  std::cout << "to eprt: " << s << std::endl;
  
  EndpointFromEPRT(s, ep);
  std::cout << "from eprt: " << ep << std::endl;

  EndpointToEPRT(ep, s, false);
  std::cout << "to eprt: " << s << std::endl;
}
