...
class User : public acl::User, private Unlockable
{
...
public:
  ...
  {
    UnlockGuard unlock(Unlocker()); (void) unlock;
  ...
  {
    return Load<Unlocker>(acl::NameToUID(name));
  }
  ...
  template <typename Unlocker>
  static std::vector<acl::UserID> GetUIDs() { return GetUIDs<Unlocker>("*"); }
  ...
  static std::vector<User> GetUsers(const std::string& multiStr)
  {
    ...
    {
      users2.push_back(std::move(user));
    }
    return users2;
  }
  
  template <typename Unlocker>
  static std::vector<User> GetUsers() { return GetUsers<Unlocker>("*"); }
